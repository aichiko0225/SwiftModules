        ...
        case .count(let count):
            if count > 0, let string = badgeView.text {
                if count >= 10 {
                    ...
                    let preferredRect = (string as NSString).boundingRect(with: size, options: [.usesLineFragmentOrigin, .usesFontLeading], attributes: [NSAttributedString.Key.font: font], context: nil)
                    let preferredRect_0 = ("0" as NSString).boundingRect(with: size, options: [.usesLineFragmentOrigin, .usesFontLeading], attributes: [NSAttributedString.Key.font: font], context: nil)
                    width = ceil(preferredRect.width)+(height-ceil(preferredRect_0.width))
                } else {
                    width = height
                }
            } else {
